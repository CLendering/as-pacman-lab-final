...
parser.add_argument(
    ...
    dest="teams",
    nargs="+",
    help="Set teams to filter by <team1> [team2]",
)
parser.add_argument(
    "-n",
    "--number",
...
parser.add_argument(
    ...
    type=float,
    dest="delay_step",
    help="Delay step in a play or replay.",
    default=0.03,
)
...
def main():
    ...
    if not os.path.exists(REPLAYS_FOLDER):
        print(
            f'No replays folder found with path "{REPLAYS_FOLDER}". This can be edited in the script under REPLAYS_FOLDER'
        )
        exit(1)
    ...
    all_files = [
        ...
        if os.path.isfile(os.path.join(DIR_SCRIPT, REPLAYS_FOLDER, f))
    ]
    ...
    if len(files) == 0:
        ...
        if (
            args.number < len(files) + 1
        ):  # Compensate for the fact that indexes start from 0 but IDs start from 1
            found_file = files[args.number - 1]
            os.system(
                generate_cmd(os.path.join(DIR_SCRIPT, REPLAYS_FOLDER, found_file))
            )
        else:
            print(
                f"ERROR: Invalid replay ID. {len(files)} replays found (select from 1 - {len(files)})."
            )
